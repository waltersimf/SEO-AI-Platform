...
### 4.6. Dual-LLM Architecture (v2.0 Vision)

**Статус:** Planned (після досягнення Product-Market Fit)  
**Очікувана економія:** 60-70% витрат на AI

#### Концепція

Гібридна архітектура з двома LLM для оптимізації вартості та швидкості:

| LLM | Призначення | Модель | Вартість |
|-----|-------------|--------|----------|
| **Internal** | Управління системою, робота з даними | Llama 3.1 8B (fine-tuned) | ~$0.001/запит |
| **External** | Web search, контент, складний аналіз | Claude API | ~$0.01/запит |
...
│  (Llama 3.1)  │        │   (Claude)    │
│               │        │               │
│ • Task CRUD   │        │ • Web search  │
│ • Data query  │        │ • Content gen │
│ • Navigation  │        │ • Analysis    │
│ • Reports     │        │ • Creative    │
│ • System mgmt │        │ • Research    │
│               │        │               │
│  ~75% запитів │        │  ~25% запитів │
└───────────────┘        └───────────────┘
```

#### Розподіл запитів

| Тип запиту | Приклад | LLM | % |
|------------|---------|-----|---|
| Task management | "Створи задачу на аудит" | Internal | 25% |
| Data queries | "Який трафік за місяць?" | Internal | 20% |
| System control | "Перенеси задачу на п'ятницю" | Internal | 15% |
| Report generation | "Згенеруй звіт по ключових" | Internal | 15% |
| Web search | "Знайди конкурентів в ніші" | External | 10% |
| Content creation | "Напиши meta description" | External | 10% |
| Complex analysis | "Проаналізуй стратегію" | External | 5% |

#### Економіка (приклад: 1000 користувачів, 50 запитів/день)

**Без оптимізації (все на Claude):**
```
50,000 запитів × $0.01 = $500/день = $15,000/міс
```

**З Dual-LLM:**
```
Internal (75%): 37,500 × $0.001 = $37.50/день
External (25%): 12,500 × $0.01  = $125/день
─────────────────────────────────────────────
Total: $162.50/день = ~$5,000/міс

Економія: $10,000/міс (66%)
```

#### Router Implementation
```typescript
function routeQuery(query: string): 'internal' | 'external' {
  const internalPatterns = [
    /створи (задачу|таск|проект)/i,
    /покажи (статистику|дані|звіт)/i,
    /перенеси|видали|оновив/i,
    /скільки|який|яка|коли/i,
    /трафік|позиції|ключові|бекл[іи]нки/i,
  ];
  
  const externalPatterns = [
    /знайди в інтернеті|пошукай|дослід/i,
    /напиши (статтю|текст|контент|опис)/i,
    /проаналізуй (конкурент|сайт|стратегію|ринок)/i,
    /створи (контент|стратегію)/i,
  ];
  
  if (externalPatterns.some(p => p.test(query))) return 'external';
  if (internalPatterns.some(p => p.test(query))) return 'internal';
  return 'internal'; // default — дешевше
}
```
...
| Перевага | Опис |
|----------|------|
| **Економія 60-70%** | Більшість запитів на дешевій internal LLM |
| **Швидкість** | Internal LLM швидша (менша модель) |
| **Приватність** | Дані задач/проектів не йдуть до зовнішніх API |
| **Надійність** | Якщо Claude API down — базовий функціонал працює |
| **Масштабування** | Internal легше масштабувати |
| **Включено в підписку** | Можна прибрати BYOK — AI в ціні |

#### Етапи впровадження

**Етап 1 (v1.0):** BYOK Claude API — поточна модель  
**Етап 2 (v1.5):** Збір даних, аналіз патернів використання  
**Етап 3 (v2.0):** Fine-tune Llama на зібраних даних  
**Етап 4 (v2.5):** Гібридна архітектура в production  
**Етап 5 (v3.0):** AI включений в базову підписку  

#### Технічні вимоги для Internal LLM

**Модель:** Llama 3.1 8B або Qwen 2.5 7B (fine-tuned)  
**Fine-tuning:** LoRA/QLoRA на власному dataset  
**Хостинг:** Together.ai, Replicate, або власний GPU  
**Latency target:** <500ms для простих запитів  
**Dataset:** ~10,000+ прикладів з реального використання
